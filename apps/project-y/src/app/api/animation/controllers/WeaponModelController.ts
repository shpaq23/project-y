...
export class WeaponModelController {
  ...
  private resolveAnimation(
    ...
    if (
      this.currentNormalAnimations[0].playing ||
      this.currentBigAnimation[1].playing
    ) {
      return;
    }

    if (this.weaponAnimationModel.universal) {
      this.currentNormalAnimations[0].textures =
        this.weaponAnimationModel.universal.normal[animation].animation[
          direction
        ].textures;
      this.copySpriteProperties(
        this.currentNormalAnimations[0],
        this.weaponAnimationModel.universal.normal[animation].animation[
          direction
        ]
      );

      this.currentNormalIdle[0].texture =
        this.weaponAnimationModel.universal.normal[animation].idle[
          direction
        ].texture;
      this.copySpriteProperties(
        this.currentNormalIdle[0],
        this.weaponAnimationModel.universal.normal[animation].idle[direction]
      );

      if (this.weaponAnimationModel.universal.behind) {
        this.currentNormalAnimations[1].textures =
          this.weaponAnimationModel.universal.behind[animation].animation[
            direction
          ].textures;
        this.copySpriteProperties(
          this.currentNormalAnimations[1],
          this.weaponAnimationModel.universal.behind[animation].animation[
            direction
          ]
        );

        this.currentNormalIdle[1].texture =
          this.weaponAnimationModel.universal.behind[animation].idle[
            direction
          ].texture;
        this.copySpriteProperties(
          this.currentNormalIdle[1],
          this.weaponAnimationModel.universal.behind[animation].idle[direction]
        );
      }
      if (!initial) {
        this.playNormal();
    ...
    if (weaponBigAnimationModel) {
      this.currentBigAnimation[0].textures =
        weaponBigAnimationModel.normal.animation[direction].textures;
      this.copySpriteProperties(
        this.currentBigAnimation[0],
        weaponBigAnimationModel.normal.animation[direction]
      );

      this.currentBigIdle[0].texture =
        weaponBigAnimationModel.normal.idle[direction].texture;
      this.copySpriteProperties(
        this.currentBigIdle[0],
        weaponBigAnimationModel.normal.idle[direction]
      );

      if (weaponBigAnimationModel.behind) {
        this.currentBigAnimation[1].textures =
          weaponBigAnimationModel.behind.animation[direction].textures;
        this.copySpriteProperties(
          this.currentBigAnimation[1],
          weaponBigAnimationModel.behind.animation[direction]
        );

        this.currentBigIdle[1].texture =
          weaponBigAnimationModel.behind.idle[direction].texture;
        this.copySpriteProperties(
          this.currentBigIdle[1],
          weaponBigAnimationModel.behind.idle[direction]
        );
      }
      if (!initial) {
        this.playBig();
  ...
  private copySpriteProperties(
    copyTo: Sprite | AnimatedSprite,
    copyFrom: Sprite | AnimatedSprite
  ): void {
    ...
    copyTo.y = copyFrom.y;
    if (
      copyFrom instanceof AnimatedSprite &&
      copyTo instanceof AnimatedSprite
    ) {
