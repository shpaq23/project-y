...
export class BodyModelController {
  ...
  private readonly currentAnimatedSprites: Array<AnimatedSprite> =
    this.bodyAnimationModel.walk.animation.down.map((sprite) => {
      return this.spriteCreator.copySprite(sprite) as AnimatedSprite;
    });
  private readonly currentIdleSprite: Array<Sprite> =
    this.bodyAnimationModel.walk.idle.down.map((sprite) => {
  ...
  private resolveAnimation(
    ...
    for (let i = 0; i < this.currentAnimatedSprites.length; i++) {
      ...
      textures = this.bodyAnimationModel[animation].animation[direction][i]
        .textures as Array<Texture>;
