...
import {
  BodyAnimationModel,
  BodyAnimationModelDirection,
} from './model/BodyAnimationModel';
import {
  WeaponAnimationBodyModel,
  WeaponAnimationDirectionModel,
  WeaponAnimationModel,
} from './model/WeaponAnimationModel';
...
export class AnimationLoader {
  ...
  constructor(private readonly spriteCreator: SpriteCreator) {}
  ...
  private getWeaponAnimationModel(look: CharacterLook): WeaponAnimationModel {
    ...
    if (weapon.universal) {
      const textures: AnimationTexturePack = this.getAnimationTexturePack(
        weapon.universal!.normal
      );
      ...
      if (weapon.universal.behind) {
        const textures: AnimationTexturePack = this.getAnimationTexturePack(
          weapon.universal!.behind!
        );
        weaponAnimationModel.universal!.behind =
          this.getWeaponAnimationBodyModel(textures, 'behind');
      }
    }

    if (weapon.walk) {
      const textures: DirectionTexturePack = this.getDirectionTexturePack(
        weapon.walk!.normal,
        this.WALK_FRAMES,
        0,
        weapon.walk!.size
      ...
      weaponAnimationModel.walk = {
        normal: this.getWeaponAnimationDirectionModel(
          textures,
          'front',
          weapon.walk.size
        ),
      ...
      if (weapon.walk.behind) {
        ...
        weaponAnimationModel.walk!.behind =
          this.getWeaponAnimationDirectionModel(
            textures,
            'behind',
            weapon.walk.size
          );
    ...
    if (weapon.slash) {
      ...
      if (weapon.slash.behind) {
        ...
        weaponAnimationModel.slash!.behind =
          this.getWeaponAnimationDirectionModel(
            textures,
            'behind',
            weapon.slash.size
    ...
    if (weapon.thrust) {
      ...
      if (weapon.thrust.behind) {
        const textures: DirectionTexturePack = this.getDirectionTexturePack(
          weapon.thrust!.behind!,
          this.THRUST_FRAMES,
          0,
          weapon.thrust!.size
        );
        weaponAnimationModel.thrust!.behind =
          this.getWeaponAnimationDirectionModel(
            textures,
            'behind',
            weapon.thrust.size
  ...
  private getWeaponAnimationBodyModel(
    textures: AnimationTexturePack,
    layer: Layer
  ): WeaponAnimationBodyModel {
    return Object.values(Animation).reduce((acc, animation) => {
      acc[animation] = this.getWeaponAnimationDirectionModel(
        textures[animation],
        layer
      );
      return acc;
  ...
  private getWeaponAnimationDirectionModel(
    textures: DirectionTexturePack,
    layer: Layer,
    oversize?: number
  ): WeaponAnimationDirectionModel {
    return {
      animation: Object.values(AnimationDirection).reduce((acc, direction) => {
        acc[direction] = this.spriteCreator.createAnimatedSprite(
          textures.animation[direction],
          layer,
          oversize
        );
        return acc;
      ...
      idle: Object.values(AnimationDirection).reduce((acc, direction) => {
        acc[direction] = this.spriteCreator.createSprite(
          textures.idle[direction],
          layer,
          oversize
        );
        return acc;
  ...
  private getBodyAnimationModel(look: CharacterLook): BodyAnimationModel {
    ...
    const bodyAnimationModel: BodyAnimationModel = Object.values(
      Animation
    ).reduce((acc, animation) => {
      acc[animation] = {
        animation: Object.values(AnimationDirection).reduce(
          (acc, direction) => {
            acc[direction] = Object.values(BodyPart)
              .filter((part) => bodyTextures[part])
              .map((part) =>
                this.spriteCreator.createAnimatedSprite(
                  bodyTextures[part]![animation].animation[direction],
                  'normal'
                )
              );
            return acc;
        ...
        idle: Object.values(AnimationDirection).reduce((acc, direction) => {
          acc[direction] = Object.values(BodyPart)
            .filter((part) => bodyTextures[part])
            .map((part) =>
              this.spriteCreator.createSprite(
                bodyTextures[part]![animation].idle[direction],
                'normal'
              )
            );
          return acc;
  ...
  private getDirectionTexturePack(
    url: string,
    frames: number,
    startFrame: number,
    frameSize: number
  ): DirectionTexturePack {
    ...
    for (let i = 0; i < frames; i++) {
      if (i === 0) {
        idleUp = new Texture(
          sheet,
          new Rectangle(0, startFrame * frameSize, frameSize, frameSize)
        );
        idleLeft = new Texture(
          sheet,
          new Rectangle(0, (startFrame + 1) * frameSize, frameSize, frameSize)
        );
        idleDown = new Texture(
          sheet,
          new Rectangle(0, (startFrame + 2) * frameSize, frameSize, frameSize)
        );
        idleRight = new Texture(
          sheet,
          new Rectangle(0, (startFrame + 3) * frameSize, frameSize, frameSize)
        );
        continue;
      }
    ...
    for (let i = 0; i < this.DEAD_FRAMES; i++) {
      if (i === 0) {
        idle = new Texture(
          ...
          new Rectangle(
            0,
            this.DEAD_FRAMES_START * this.FRAME_SIZE,
            this.FRAME_SIZE,
            this.FRAME_SIZE
          )
        );
  ...
  private getAnimationTexturePack(url: string): AnimationTexturePack {
    ...
    const walk = this.getDirectionTexturePack(
      url,
      this.WALK_FRAMES,
      this.WALK_FRAMES_START,
      this.FRAME_SIZE
    );
    const slash = this.getDirectionTexturePack(
      url,
      this.SLASH_FRAMES,
      this.SLASH_FRAMES_START,
      this.FRAME_SIZE
    );
    const thrust = this.getDirectionTexturePack(
      url,
      this.THRUST_FRAMES,
      this.THRUST_FRAMES_START,
      this.FRAME_SIZE
    );
    const shoot = this.getDirectionTexturePack(
      url,
      this.SHOOT_FRAMES,
      this.SHOOT_FRAMES_START,
      this.FRAME_SIZE
    );
    const cast = this.getDirectionTexturePack(
      url,
      this.CAST_FRAMES,
      this.CAST_FRAMES_START,
      this.FRAME_SIZE
    );
