...
export class DOMManipulator {
	...
	hostListen<E extends keyof GlobalEventHandlersEventMap>(eventName: E): Observable<EventWith<GlobalEventHandlersEventMap[E], HTMLElement>>;
	...
	listen<
		...
		T extends FromEventTarget<
			EventWith<GlobalEventHandlersEventMap[E], T>
		>
	>(nativeElement: T, eventName: E): Observable<EventWith<GlobalEventHandlersEventMap[E], T>>;
	listen<E extends keyof WindowEventHandlersEventMap>(nativeElement: Window, eventName: E): Observable<EventWith<WindowEventHandlersEventMap[E], Window>>;
	listen<T extends Event>(nativeElement: HTMLElement | Window | Document, eventName: string): Observable<T>;
	listen<T extends Event>(nativeElement: HTMLElement | Window | Document, eventName: string): Observable<T> {
	...
	private addClasses(element: HTMLElement, classes: Array<string>): void {
		element.classList && element.classList.add(
			...classes
				.filter(Boolean)
				.map(c => c.split(' '))
...
export type EventWith<E extends Event,
	T extends FromEventTarget<E>> = E & {
